...
public final class RoadPricingReaderXMLv2 extends MatsimXmlParser  {
	...
	public RoadPricingReaderXMLv2(RoadPricingSchemeImpl scheme){
		this.scheme = scheme;
	}
		...
		if (TAG_ROADPRICING.equals(name)) {
			...
			if (TAG_COST.equals(name) && this.currentLinkId == null) {
				this.scheme.createAndAddCost(Time.parseTime(atts.getValue(ATTR_START_TIME)),
						endTime, Double.parseDouble(atts.getValue(ATTR_AMOUNT)), atts.getValue(ATTR_MODE));
