...
public final class CalcPaidToll implements LinkEnterEventHandler, VehicleEntersTrafficEventHandler, VehicleLeavesTrafficEventHandler {
	...
	public Map<String, ModeTollInfo> getLegTolls() {
		return legTolls;
	}

	private final HashMap<String, ModeTollInfo> legTolls = new HashMap<String, ModeTollInfo>();
	...
	private Map<Id<Person>, Integer> currentActivityIndx = new HashMap<>();

    @Inject
	CalcPaidToll(final Scenario scenario, final RoadPricingScheme scheme, EventsManager events ) {
		super();
	...
	public void handleEvent(final VehicleEntersTrafficEvent event) {
		if(!currentActivityIndx.containsKey(event.getPersonId())) {
			currentActivityIndx.put(event.getPersonId(), 0);
		}
		Leg leg = PopulationUtils.getLegs(scenario.getPopulation().getPersons().get(event.getPersonId()).getSelectedPlan()).get(currentActivityIndx.get(event.getPersonId()));
		if(!legTolls.containsKey(leg.getMode())) {
			legTolls.put(leg.getMode(), new ModeTollInfo());
		}
		LegInfo.addCurrentLeg(event.getPersonId(), leg);
...
//	}
	...
	public void reset(final int iteration) {
		this.agents.clear();
		this.currentActivityIndx.clear();
		this.legTolls.clear();
	...
	class DistanceTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			Leg leg = LegInfo.getCurrentLeg(driverId);
			Cost cost = CalcPaidToll.this.scheme.getLinkCostInfo(link.getId(),
					event.getTime(), driverId, event.getVehicleId(), leg.getMode());
			if (cost != null && leg.getMode().equals(cost.mode)) {
				double newToll = link.getLength() * cost.amount;
				AgentTollInfo info = CalcPaidToll.this.agents.get(driverId);
				CalcPaidToll.this.legTolls.get(leg.getMode()).incAmount(cost.amount);
				if (info == null) {
				...
				legTolls.get(leg.getMode()).incAmount(cost.amount);
				info.toll += newToll;
	...
	class LinkTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			Id<Person> driverId = delegate.getDriverOfVehicle(event.getVehicleId());
			Leg leg = LegInfo.getCurrentLeg(driverId);
			Cost cost = CalcPaidToll.this.scheme.getLinkCostInfo(link.getId(), event.getTime(), driverId, event.getVehicleId(), leg.getMode());
			if (cost != null && leg.getMode().equals(cost.mode)) {

				AgentTollInfo info = CalcPaidToll.this.agents.get(event.getVehicleId());
				if (info == null) {
					/* The agent is not yet "registered" */
	...
	class CordonTollBehaviour implements TollBehaviourI {
		private EventsManager events;
		private boolean freeMovementInsideCordon = true;
		/**
		 * @param events The EventsManager to send money events
		 */
		public CordonTollBehaviour(EventsManager events) {
			...
			if(rpConfig.getFreeMovementInsideCordon()!=null) {
				this.freeMovementInsideCordon = Boolean.parseBoolean(rpConfig.getFreeMovementInsideCordon());
			}
		}
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			Cost cost = CalcPaidToll.this.scheme.getLinkCostInfo(link.getId(), event.getTime(), driverId, event.getVehicleId(), leg.getMode());
			if (cost != null && leg.getMode().equals(cost.mode)) {
				//log.warn("PAYMENT: linkId: "+link.getId()+", mode: "+leg.getMode() + ", amount: "+cost.amount);
				...
				if (info == null) {
					...
					if(this.freeMovementInsideCordon) {
	...
	public void handleEvent(VehicleLeavesTrafficEvent event) {
		delegate.handleEvent(event);
		currentActivityIndx.merge(event.getPersonId(), 1, Integer::sum);
	}
...
class ModeTollInfo {
	...
	public void incAmount(double cost) {
		this.amount+=cost;
	}
}
