...
public final class RoadPricingSchemeImpl implements RoadPricingScheme {
	 ...
	 */
	public Cost createAndAddCost(final double startTime, final double endTime, final double amount, final String mode) {
		...
		Cost cost = new Cost(startTime, endTime, amount, mode);
		this.costs.add(cost);
		this.cacheIsInvalid = true;
		return cost;
	}

	public void addLinkCost(Id<Link> linkId, double startTime, double endTime, double amount, String mode) {
		if ( startTime==0. && endTime == 24.*3600. ) {
			if (wrnCnt < 1) {
				wrnCnt++ ;
				log.warn("startTime=0:00 and endTime=24:00 means NO toll after 24h (no wrap-around); make sure this is what you want" ) ;
	...
	@Override
	public Cost getLinkCostInfo(final Id<Link> linkId, final double time, Id<Person> personId, Id<Vehicle> vehicleId, String mode) {
		...
		if (this.linkIds.containsKey(linkId)) {
			...
			if (linkSpecificCosts == null) {
				...
				for (Cost cost : this.costCache) {
					if ((time >= cost.startTime) && (time < cost.endTime) && (mode.equals(cost.mode))) {
						return cost;
					}
				}
			}
	...
	public Cost getTypicalLinkCostInfo(Id<Link> linkId, double time, String mode) {
		return this.getLinkCostInfo( linkId, time, null, null, mode) ;
	}
	...
	static public class Cost {
		...
		public Cost(final double startTime, final double endTime, final double amount, String mode) {
		...
		public String toString() {
			return "startTime: " + this.startTime + " endTime: " + this.endTime + " amount: " + this.amount + " mode: " +this.mode ;
		}
	}
}
