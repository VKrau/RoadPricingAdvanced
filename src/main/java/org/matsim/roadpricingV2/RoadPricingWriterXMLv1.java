...
/**
 * Writes a {@link org.matsim.roadpricingV2.RoadPricingSchemeImpl} to a file according to <code>roadpricing_v1.dtd</code>.
 *
 * @author mrieser
 */
public final class RoadPricingWriterXMLv1 extends MatsimXmlWriter {
	...
	public void writeFile(final String filename) throws UncheckedIOException {
		...
		writeDoctype("roadpricing", "roadpricingAdv.dtd");
		write();
		close();
	}
	...
	private void write() throws UncheckedIOException {
		try {
			...
			for (Cost cost : this.scheme.getTypicalCosts()) {
			  this.writeCost(cost);
			}
    ...
    this.writer.write("mode=\"" +cost.mode + "\" />\n");
	}
