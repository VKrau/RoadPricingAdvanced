...
class TravelDisutilityIncludingToll implements TravelDisutility {
	...
	public double getLinkTravelDisutility(final Link link, final double time, final Person person, final Vehicle vehicle)
	{
		...
		// end randomize
		Leg leg = LegInfo.getCurrentLeg(person.getId());
		Double tollCost = null;
		if(leg!=null) {
			 tollCost = this.tollCostHandler.getTypicalTollCost(link, time, leg);
		} else {
			Leg firstLeg = PopulationUtils.getLegs(person.getSelectedPlan()).get(0);
			tollCost = this.tollCostHandler.getTypicalTollCost(link, time, firstLeg);
		}
	...
	private interface TollRouterBehaviour {
		public double getTypicalTollCost(Link link, double time, Leg leg);
	}

	/*package*/ class DistanceTollCostBehaviour implements TollRouterBehaviour {
		@Override
		public double getTypicalTollCost(final Link link, final double time, final Leg leg) {
			Cost cost_per_m = scheme.getTypicalLinkCostInfo(link.getId(), time, leg.getMode());
			if (cost_per_m == null) {
				return 0.0;
			}
	...
	/*package*/ class AreaTollCostBehaviour implements TollRouterBehaviour {
		@Override
		public double getTypicalTollCost(final Link link, final double time, final Leg leg) {
			Cost cost = scheme.getTypicalLinkCostInfo(link.getId(), time, leg.getMode());
			if (cost == null) {
				return 0.0;
			}
